...
Make the relevant ClickUp call. If the tool is unavailable / the server is not connected, stop and tell the user:
...
- **Ingest mode** — the input is only a task ID/URL (or `Clickup ID:` line) with no instruction, or it asks to "pull / get context / start work on" a task. → **Part A**.
- **Action mode** — the input contains an instruction to change the task. Trigger verbs: *add comment, reply, comment, notify, mark as, move to, set/change status, assign, log time, track time, start timer, stop timer*. → **Part B**.
...
```
Capture `name`, `markdown_description` (**use this, not `text_content`** — `text_content` strips image links), `status`, `priority`, `tags`, `assignees`, `task_type`, `list`, `folder`, `space`, `url`, `custom_fields`, `checklists`, `linked_tasks`, `dependencies`, and `subtasks`. If an ID errors, report it and continue with the rest.

## A2: Resolve scope
- **One leaf task** → it is the feature/bug. Proceed.
- **Multiple IDs** → `AskUserQuestion`: **Merge into one feature** (single context doc) or **Treat as separate features** (one folder each; report a suggested order at the end).
- **Parent with subtasks** → parent is the umbrella; choose subtasks to deep-fetch (A3).

## A3: Deep-fetch subtasks (when present)
The `subtasks` array has each child's `id`, `name`, `status`, `task_type` — but **empty `description`**. To get a subtask's body and mockups, call `clickup_get_task(subtask_id, include: ["description"])`.
...
4. If a download fails (non-200 / 0 bytes), report the URL and keep its link in the context doc.
If there are no images, note it and move on.

## A6: Set up the artifact folder
Derive a kebab-case feature name from the title (`Build UI for PIN Code TAT check` → `pin-code-tat`).
```bash
mkdir -p .buildspace/artifacts/{feature}/clickup-images
...
Read `${CLAUDE_SKILL_DIR}/templates/clickup-context-template.md` and fill it with the real data. Write to `.buildspace/artifacts/{feature}/clickup-context.md`. One block per source task; for a parent, parent first then a block per deep-fetched subtask. Reference downloaded images by path. Omit empty rows — never invent values.

## A8: Classify and route
`task_type == "Bug"` → bug; otherwise → feature; if ambiguous, ask. Then:
...
**Batch:** list each folder + suggested order; note `current-feature` is unset until they choose.
...
These actions change a **shared** task others can see. **Always: draft → confirm with `AskUserQuestion` → write.** Never write without explicit confirmation. Do each requested action in order, each with its own confirmation.
...
3. **Missing references:** if the comment mentions something you don't have (e.g. "here is the link" but no URL was given), ask for it, or offer to pull it from the feature's artifacts (preview/PR URL in `execution-log.md`). **Never invent a link.**
4. Show the final comment text + target task + notify setting, and confirm.
...
2. Map the requested status to a valid value (e.g. "QA" → `qa`, "done" → `complete`). If it matches none, show the valid options and ask.
3. For assignees, resolve names/emails/"me" via `clickup_resolve_assignees` first.
4. Confirm the change (old → new), then `clickup_update_task(task_id, status: "...", assignees?, priority?)`.

## Log / track time
- **Live timer:** "start timer" → `clickup_start_time_tracking(task_id)`; "stop timer" → `clickup_stop_time_tracking()`. Only one timer runs at a time. (No timestamps needed — preferred for real-time work.)
- **Manual entry:** `clickup_add_time_entry(task_id, start, duration | end_time)`. `start`/`end_time` must be `YYYY-MM-DD HH:MM`. You do **not** reliably know the current clock time — if the user gives only a duration ("log 2h"), ask for the start time (or start+end). Never guess a timestamp.
...
- Ingest writes only local artifacts; Action writes to the shared ClickUp task — Action always confirms first
...
- Subtask list view has empty descriptions — deep-fetch a subtask by ID for its body
- ClickUp attachment URLs are public — download with plain `curl`, no token
- A literal `@Name` is not a real mention — resolve + notify_all/assignee to actually notify
